...
const BASE=48, COUNT=32, TOP=BASE+COUNT-1;
...
const LESSONS=[
 ...
 {id:'full-range',name:'Przejazd C3-G5',bpm:96,notes:Array.from({length:32},(_,i)=>BASE+i)}
];
...
function laneOf(note){return note-BASE}
function inRange(note){return note>=BASE&&note<=TOP}
...
function bind(){ $('audioBtn').onclick=initAudio;$('midiBtn').onclick=connectMidi;$('startBtn').onclick=start;$('stopBtn').onclick=()=>stop();$('previewBtn').onclick=preview;$('lessonSelect').onchange=()=>{stop(false);$('tempoInput').value=selectedLesson().bpm;setupNotes();status(selectedLesson().name)};$('tempoInput').oninput=()=>{if(state.mode!=='idle')stop(false);setupNotes()};document.addEventListener('keydown',e=>{if(e.repeat)return;let key=[...document.querySelectorAll('.key')].find(k=>k.dataset.hotkey===e.key.toLowerCase());if(key)handleNoteOn(+key.dataset.note,1)});document.addEventListener('keyup',e=>{let key=[...document.querySelectorAll('.key')].find(k=>k.dataset.hotkey===e.key.toLowerCase());if(key)handleNoteOff(+key.dataset.note)})}
